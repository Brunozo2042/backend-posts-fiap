...
export const createUser = async(req, res)=>{
    ...
    if(!name || !username || !password ||!email || !isActive){
       return res
         .status(400)
         .json({error: `Os valores: name, username, password, email, isActive, 
            São obrigatórios!`}); 
    }
    try{
         const newUser = await User.create({name, username, password, email, createdAt, isActive})
...
export const deleteUser = async(req, res) => {
    try{
       ...
       if(!deleted){
        ...
        res.json({messsage:"User removido com sucesso", user: deleted});
